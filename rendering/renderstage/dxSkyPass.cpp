...
void rendering::renderstage::DXSkyPass::InitProperties(interpreter::NativeObject & nativeObject)
{
...
bool rendering::renderstage::DXSkyPass::Init(DXRenderer& renderer, std::string& errorMessage)
{
...
bool rendering::renderstage::DXSkyPass::Execute(DXRenderer& renderer, std::string& errorMessage)
{
    THROW_ERROR(
...
bool rendering::renderstage::DXSkyPass::RenderMaterials(rendering::DXRenderer& renderer, std::string& errorMessage)
{
    ...
    for (std::map<std::string, scene::Object3D>::iterator it = scene->m_objects.begin(); it != scene->m_objects.end(); ++it) {
        ...
        for (std::list<std::string>::const_iterator materialNameIt = cur.m_materials.begin(); materialNameIt != cur.m_materials.end(); ++materialNameIt) {
            ...
            if (materialIt->second->GetMaterialType() == material::MaterialType::Sky) {
