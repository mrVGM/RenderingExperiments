...
{
    float2 centerOffset = 2 * (uv - 0.5);
    centerOffset *= m_factor / (0.6 * m_factor.y);
    float2 dir = normalize(centerOffset);
    float d = length(centerOffset);

    float noiseScale = cos(m_verticalOffset) + 1;
    noiseScale /= 2;
    noiseScale = (1 - noiseScale) * 1 + noiseScale * m_noiseScale;

    float2 noiseCoord = noiseScale * dir + 0.5;
    float3 uvw = float3(noiseCoord, m_verticalOffset);
