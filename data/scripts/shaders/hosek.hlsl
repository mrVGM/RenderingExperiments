...
static const float kHosekCoeffsX[] = {
	-1.171419,
	-0.242975,
	-8.991334,
	9.571216,
...
static const float kHosekCoeffsY[] = {
	-1.185983,
	-0.258118,
	-7.761056,
	8.317053,
...
static const float kHosekCoeffsZ[] = {
	-1.354183,
	-0.513062,
	-42.192680,
	42.717720,
...
float F(float theta, float gamma, in float coeffs[9]) {
	float A = coeffs[0];
	float B = coeffs[1];
	float C = coeffs[2];
	float D = coeffs[3];
    ...
    float3 objectScale : OBJECT_SCALE)
{
    ...
    pos += result.position.x * horizontalScale * m_camRight.xyz + result.position.y * verticalScale * m_camUp.xyz;
    result.world_pos = pos;

    return result;
}

float4 PSMain(float4 position : SV_POSITION, float3 world_pos : WORLD) : SV_Target
{
	float sun_zenith = 0;
	float sun_azimuth = 0;
	...
	float3 viewDir = world_pos - m_camPos.xyz;
	viewDir = normalize(viewDir);
	view_zenith = acos(dot(viewDir, float3(0, 1, 0)));
	...
	view_azimuth = acos(viewProj.x);
	if (viewProj.z < 0) {
		view_azimuth = 2 * M_PI - view_azimuth;
	}

	float3 sunProj = sunDir;
	sunProj.y = 0;
	sun_azimuth = acos(sunProj.x);
	if (sunProj.z < 0) {
		sun_azimuth = 2 * M_PI - sun_azimuth;
	}

	float3 XYZ = sample_sky(view_zenith, view_azimuth, sun_zenith, sun_azimuth);
	float3 RGB = XYZ_to_RGB(XYZ);

	float3 col = expose(RGB, 1);
	col = float3(col.y, col.z, col.x);
